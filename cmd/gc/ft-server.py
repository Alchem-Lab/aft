...
def setup_pod(client, internal_ip):
    ...
    with open('replicas.txt', 'w') as f:
        for ip in ips:
            f.write(ip + '\n')
    os.system('kubectl cp replicas.txt %s:/go/src/github.com/Alchem-Lab/aft' % pname)
    os.system('kubectl cp ../config/aft-config.yml %s:/go/src/github.com/Alchem-Lab/aft/config' % pname)
